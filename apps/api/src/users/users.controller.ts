...
export class UsersController {
    ...
    @ApiQuery({ name: 'filter', required: false, description: 'Texto para filtrar usuários' })
    ...
    @ApiResponse({ status: 200, description: 'Usuário atualizado parcialmente com sucesso' })
    @ApiResponse({ status: 400, description: 'Dados inválidos' })
    @ApiResponse({ status: 404, description: 'Usuário não encontrado' })
    @ApiBearerAuth()
    @Patch(':id')
