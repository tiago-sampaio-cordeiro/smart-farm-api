...
export class FarmsController {
    ...
    @ApiResponse({ status: 200, description: 'Lista de plantações retornada com sucesso' })
    @ApiBearerAuth()
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles('USER')
    @Get()
    ...
    @ApiBody({ type: CreateFarmDto })
    @ApiOperation({ summary: 'Atualizar completamente uma plantação pelo cuid()' })
    ...
    @ApiResponse({ status: 200, description: 'Plantação atualizada parcialmente com sucesso' })
    @ApiResponse({ status: 400, description: 'Dados inválidos' })
    @ApiResponse({ status: 404, description: 'Plantação não encontrada' })
    @ApiBearerAuth()
    @UseGuards(JwtAuthGuard, RolesGuard)
