...
export class SensorStatusService {
    ...
    async checkInactiveSensors() {
        const inactivityMinutes = parseInt(process.env.SENSOR_INACTIVITY_MINUTES || '5');
        const inactivityThreshold = new Date(Date.now() - inactivityMinutes * 60 * 1000);

        this.logger.log(`Verificando sensores inativos (limite: ${inactivityMinutes} minutos)`);
