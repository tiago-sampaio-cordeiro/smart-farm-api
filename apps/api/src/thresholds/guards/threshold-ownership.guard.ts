...
export class ThresholdOwnershipGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        // Para rotas /thresholds/farm/:farmId - verifica a farm direto
        const farmId = request.params.farmId;
        if (farmId) {
            const farm = await this.prisma.farm.findUnique({ where: { id: farmId } });
            if (!farm || farm.userId !== userId) {
                throw new ForbiddenException('Você não tem permissão para acessar esta lavoura.');
            }
            return true;
        }
        ...
        if (thresholdId) {
            ...
            if (!threshold || threshold.farm.userId !== userId) {
                throw new ForbiddenException('Você não tem permissão para acessar este threshold.');
            }
            return true;
        }
