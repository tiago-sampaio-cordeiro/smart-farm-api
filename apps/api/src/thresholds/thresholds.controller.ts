import { Body, Controller, Delete, Get, HttpCode, Param, Post, Put, UseFilters, UseGuards } from '@nestjs/common';
...
export class ThresholdsController {
    ...
    @ApiResponse({ status: 200, description: 'Lista de parâmetros retornada com sucesso' })
    ...
    @ApiResponse({ status: 200, description: 'Parâmetros encontrados com sucesso' })
    @ApiResponse({ status: 404, description: 'Parâmetros não encontrados' })
    @ApiBearerAuth()
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles('USER')
    ...
    @ApiBody({ type: CreateTheresholdDto })
    @ApiResponse({ status: 200, description: 'Parâmetros atualizados com sucesso' })
    @ApiResponse({ status: 400, description: 'Dados inválidos' })
    @ApiResponse({ status: 404, description: 'Parâmetros não encontrados' })
    @ApiBearerAuth()
    @UseGuards(JwtAuthGuard, RolesGuard)
