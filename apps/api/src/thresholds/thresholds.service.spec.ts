...
describe('ThresholdsService', () => {
  ...
  describe('create', () => {
    ...
    it('deve lançar DuplicateThresholdException quando já existir um threshold do mesmo tipo na fazenda', async () => {
      ...
      await expect(service.create(dto as any)).rejects.toThrow(DuplicateThresholdException);
  ...
  describe('findByFarm', () => {
    it('deve retornar todos os thresholds de uma plantação', async () => {
      const thresholds = [
        { id: 'threshold-id-1', farmId: 'farm-id-1', type: 'TEMPERATURA', min: 10, max: 40 },
        { id: 'threshold-id-2', farmId: 'farm-id-1', type: 'UMIDADE', min: 30, max: 80 },
      ];
      mockPrismaService.threshold.findMany.mockResolvedValue(thresholds);

      const result = await service.findByFarm('farm-id-1');

      expect(mockPrismaService.threshold.findMany).toHaveBeenCalledWith({ where: { farmId: 'farm-id-1' } });
  ...
  describe('findOne', () => {
    it('deve retornar um threshold quando encontrado', async () => {
      const threshold = { id: 'threshold-id-1', farmId: 'farm-id-1', type: 'TEMPERATURA', min: 10, max: 40 };
      mockPrismaService.threshold.findUnique.mockResolvedValue(threshold);

      const result = await service.findOne('threshold-id-1');

      expect(mockPrismaService.threshold.findUnique).toHaveBeenCalledWith({ where: { id: 'threshold-id-1' } });
  ...
  describe('update', () => {
    ...
    it('deve lançar NotFoundException quando o threshold não existir', async () => {
      ...
      await expect(service.update('id-invalido', { max: 60 })).rejects.toThrow(NotFoundException);
  ...
  describe('remove', () => {
    it('deve remover o threshold quando encontrado', async () => {
      ...
      expect(mockPrismaService.threshold.delete).toHaveBeenCalledWith({ where: { id: 'threshold-id-1' } });
      expect(result).toEqual(threshold);
    });
