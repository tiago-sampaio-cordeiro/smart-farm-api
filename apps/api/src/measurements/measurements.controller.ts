...
export class MeasurementsController {
    ...
    @ApiOperation({ summary: 'Listar todas as medições, com filtro opcional por sensorId' })
    @ApiQuery({ name: 'sensorId', required: false, description: 'Ids dos sensores para filtrar as medições' })
    @ApiResponse({ status: 200, description: 'Lista de medições retornada com sucesso' })
    @ApiBearerAuth()
    @Get()
