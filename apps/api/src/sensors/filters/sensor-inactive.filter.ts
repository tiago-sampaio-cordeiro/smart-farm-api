...
export class SensorInactiveFilter implements ExceptionFilter {
    private readonly logger = new Logger(SensorInactiveFilter.name);
        ...
        this.logger.warn(`[${request.method}] ${request.url} - Tentativa de medição com sensor inativo.`);

        response.status(status).json({
            erro: 'Sensor Inativo',
            mensagem: exception.message,
            sugestao: 'Verifique a conexão do sensor e reative-o antes de registrar medições.',
